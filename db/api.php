...
function addNewOrder($pdo, $params) {
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO comenzi (id, client, telefon, data, termenLivrare, urgenta,  status, note, detalii, total, mod_user,mod_timestamp) VALUES (:id, :client, :telefon, :data, :termenLivrare, :urgenta, :status, :note, :detalii, :total, :mod_user,:mod_timestamp)");
        $stmt->execute([
            'id' => $orderData['id'],
            ...
            'mod_user' => $params['mod_user'], 
            'mod_timestamp' => $params['mod_timestamp'] 
        ]);
...
function updateOrder($pdo, $params) {
    try {
        ...
        $stmt = $pdo->prepare("UPDATE comenzi SET client = :client, telefon = :telefon, data = :data, termenLivrare = :termenLivrare, urgenta = :urgenta, status = :status, note = :note, detalii = :detalii, total = :total, mod_user = :mod_user, mod_timestamp = :mod_timestamp WHERE id = :id");
        $stmt->execute([
            'id' => $params['id'],
            ...
            'total' => $params['total'],
            'mod_user' => $params['mod_user'], // Assume $modUser contains the user ID or username of the modifier
            'mod_timestamp' => $params['mod_timestamp'] // Use the current date and time as the timestamp
        ]);
...
function newOrders($pdo2, $params) {
        ...
        $stmt = $pdo2->prepare("SELECT * FROM comenzi WHERE data > :oneMonthAgo");
